...
const PortfolioScreen = () => {
  ...
  return (
    <div className="max-w-5xl mx-auto mt-5">
      <LargeTable data={nndata}></LargeTable>
      <SectorGraph data={nndata}></SectorGraph>
      <SectionBenchmark></SectionBenchmark>
    </div>
  );
};
...
const transformPortfolioToDisplayFormat = (portfolioItem) => {
  ...
  const totalCost = portfolioItem.transactions.reduce(
    (acc, transaction) => acc + transaction.price * transaction.volume,
    0
  );
